...
from .selfAttn_merge import selfAttn_merge_SRNN
...
class selfAttn_merge_SRNN_lidar(selfAttn_merge_SRNN):
    ...
    def forward(self, inputs, rnn_hxs, masks, infer=False):
        ...
        expected_lidar_shape = (
            seq_length * nenv,
            self.lidar_channel_num,
            self.lidar_input_size,
        )
        if tuple(inputs['point_clouds'].shape) != expected_lidar_shape:
            raise ValueError(
                "point_clouds must have shape {}, got {}".format(
                    expected_lidar_shape,
                    tuple(inputs['point_clouds'].shape),
                )
            )
        lidar_in = inputs['point_clouds'].reshape(expected_lidar_shape)
